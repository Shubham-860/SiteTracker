...
const Users = () => {
    ...
    const cols = [
        ...
        {field: 'superuser', header: 'superuser'},
    ...
    const deleteField = async (id) => {
        // alert(id)
        try {
            await axios.delete('http://localhost:8081/deleteUser/' + Number(id))
                .then(res => {
                    console.log('res')
                    console.log(res)
                    showSuccess()
    ...
    const deleteBody = (rawData) => {
        return (
            <div className="flex justify-content-center" key={rawData.userid}>
                <BsTrash className={'deleteIcon'} onClick={() => {
                    deleteField(rawData.userid)
    ...
    const date = (rawData) => {
        const date = new Date(rawData.InsertDateTime)
        return (<div className={'p-0 m-0'}>{date.toLocaleString()} </div>)
    }
    const getDrivers = () => {
    ...
    const header = (<div className="row">
        ...
        <h2 className={'col-md-4 text-center'}>Users</h2>
    ...
    return (<div className={'container-fluid'}>
            ...
            <div className={'card m-5'}>
                <DataTable value={drivers} removableSort tableStyle={{minWidth: '50rem'}} filters={filters}
                    ...
                    <Column field={'InsertDateTime'} header={'Insert Date Time'} body={date} sortable></Column>
                    <Column field={'email'} header={'Email'} sortable></Column>
                    <Column field={'superuser'} header={'Superuser'} sortable></Column>
